...
public class TransaksiPenjualanController extends Controller {
    ...
    private void tambahDataTable() {
        try {
            ...
            ResultSet ketegorsi = DB.query("SELECT tuslah,tipe FROM `obat` JOIN kategori on obat.id_kategori = kategori.id WHERE kode_obat = '" + kdObat + "'");
            int tuslah = 0;
            String tampilantuslah = null;
            ...
            for (int row = 0; row < rowCount; row++) {
                ...
                if (namaObatTable.equals(namaObat) && satuanTable.equals(satuan) && !isEdit) {
                    ...
                    if (ketegorsi.next()) {
                        if (ketegorsi.getDouble("tuslah") == 0.00) {
                            tuslah = totalHarga;
                        } else {
                            if (ketegorsi.getString("tipe").equals("persen")) {
                                double hitung = (totalHarga * (ketegorsi.getDouble("tuslah") / 100));
                                tuslah = totalHarga + (int) hitung;
                                tampilantuslah = ketegorsi.getDouble("tuslah") + "%";
                            } else {
                                tuslah = totalHarga + ketegorsi.getInt("tuslah");
                                tampilantuslah = Helper.Currency.format(ketegorsi.getInt("tuslah"));
                            }
                        }

                    }
                    view.getTable().setValueAt(tampilantuslah, row, 5);
                    view.getTable().setValueAt(Helper.Currency.format(tuslah), row, 6);
...
//                    view.getTable().setValueAt(Helper.Currency.format(totalHarga), row, 5);
            ...
            if (!dataDitemukan) {
                ...
                Object[] data = {kdObat, namaObat, satuan, hargaFormat, qty, tampilantuslah, Helper.Currency.format(tuslah)};
                model.addRow(data);
            }
            resetForm();
            tmpData.clear();
            ...
            for (int i = 0; i < view.getTable().getRowCount(); i++) {
                ...
                if (tuslah.contains("%")) {
                    ...
                    try {
                        ...
                        // Tangani kemungkinan kesalahan parsing
                        System.out.println("Error parsing currency format: " + e.getMessage());
                        tuslah = "0"; // Atau mungkin nilai default lainnya
                    }
                } else {
                    // Setel tuslah ke "0" jika tidak mengandung tanda persen atau "Rp."
                    tuslah = "0";
                }
//                ResultSet ketegorsi = DB.query("SELECT tuslah,tipe FROM `obat` JOIN kategori on obat.id_kategori = kategori.id WHERE kode_obat = '" + kodeObat + "'");
    ...
    private void hitungTotal() {
        ...
        hargaTotal = 0;
        for (int row = 0; row < rowCount; row++) {
            hargaTotal += Helper.Currency.deformat(view.getTable().getValueAt(row, 6).toString());
        }
...
//                listData.remove(row);
        ...
        view.getTable().getColumnModel().getColumn(7).setCellRenderer(new BtnAction(false));
        view.getTable().getColumnModel().getColumn(7).setCellEditor(new BtnEditor(event, false));
